...
| `gui` `unhealthy`, але `curl http://localhost/` віддає сторінку | healthcheck `gui` перевіряє ще й `api` через proxy: `curl -s http://localhost/api/v1/health/components` → `not_ready` означає проблему в `api`, не в nginx; після відновлення `api` gui стає healthy сам (перевірено — ~20 с) |
