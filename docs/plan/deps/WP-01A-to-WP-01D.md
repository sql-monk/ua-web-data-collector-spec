...
## 7. Publisher loop: poison event до `mark_failed` (gate 4, N-2)

`outbox.fetch_unpublished` ховає рядки на visibility lease, але **не рахує доставок**. Якщо
publisher падає на конкретній події ще до `mark_failed` (crash процесу, OOM на серіалізації),
рядок повертається після lease і крутиться нескінченно: межа `max_attempts` (паркування,
CR-3) не спрацьовує. Прохання до owner-а publisher loop (WP-01D, або WP, якому його
передадуть) обрати одне з двох:

1. рахувати спроби доставки в самому loop і викликати `mark_failed` (чи паркування) до
   ризикованої операції, щоб crash не обходив лічильник;
