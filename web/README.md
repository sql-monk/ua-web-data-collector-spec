...
`sessionStorage` геттером/сеттером, який логує і кидає `TypeError`, а `src/main.tsx`
встановлює його **до першого рендеру**. Alias-обхід падає у рантаймі — і в dev, і в prod, і у
власному коді, і в будь-якій залежності з bundle. Покрито
`tests/unit/browser-storage-guard.test.ts` (5 тестів) і E2E «guard блокує обхід ESLint через
alias»; E2E також читає сховище повз сторінку (`context.storageState()`) і вимагає, щоб воно
було порожнє.
