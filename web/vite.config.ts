...
export default defineConfig(({ mode }) => ({
  ...
  test: {
    ...
    css: false,
    restoreMocks: true,
    // Компроміс за код-рев'ю L-2: глобально НЕ хвилина, але й не дефолтні 5 с — на
    // холодному кеші перший тест платить за transform/import усього дерева (виміряно 7.5 с
    // на чистому `node_modules`), і 5 с давали флакі-падіння. 20 с ловлять справжнє
    // зависання швидко. Єдиний по-справжньому повільний випадок — програмний запуск ESLint
    // з type-aware конфігом — має власний таймаут у своєму файлі, а не тут.
    testTimeout: 20_000,
  },
}));
